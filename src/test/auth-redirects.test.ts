...
describe("auth redirect safety", () => {
  it("preserva somente destinos internos", () => {
    ...
    expect(sanitizeNextPath("https://malicious.example/collect")).toBe("/dashboard");
    expect(sanitizeNextPath("//malicious.example/collect")).toBe("/dashboard");
    expect(sanitizeNextPath("/\\malicious.example")).toBe("/dashboard");
