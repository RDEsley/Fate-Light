...
returns table (
  ...
  due_soon_expenses bigint,
  expired_domains bigint,
  upcoming_domains bigint,
  active_clients bigint,
  active_client_entities bigint
)
language sql
stable
security invoker
...
as $$
  select
    ...
    (select count(*) from public.expenses where workspace_id = p_workspace_id and status = 'pending' and due_date between p_today and p_next_week),
    ...
    (select count(*) from public.client_entities
      where workspace_id = p_workspace_id and status = 'active' and archived_at is null)
...
comment on function public.dashboard_financial_summary(uuid, date, date, date, date, date, date) is
  'Agrega o dashboard no banco, incluindo despesas nos próximos 7 dias e empresas/marcas ativas.';
